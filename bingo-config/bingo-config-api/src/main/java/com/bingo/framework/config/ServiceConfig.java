...
import com.bingo.framework.config.annotation.Service;
...
import com.bingo.framework.rpc.ServiceClassHolder;
...
public class ServiceConfig<T> extends AbstractServiceConfig {
    ...
    protected synchronized void doExport() {
        ...
        if (interfaceName == null || interfaceName.length() == 0) {
            throw new IllegalStateException("<bingo:service interface=\"\" /> interface not allow null!");
        }
        checkDefault();
        if (provider != null) {
            if (application == null) {
                ...
                if (arguments != null && arguments.size() > 0) {
                    for (ArgumentConfig argument : arguments) {
                        ...
                        if (argument.getType() != null && argument.getType().length() > 0) {
                            ...
                            throw new IllegalArgumentException("argument config must set index or type attribute.eg: <bingo:argument index='0' .../> or <bingo:argument type=xxx .../>");
                        }
        ...
        if (!Constants.SCOPE_NONE.toString().equalsIgnoreCase(scope)) {
            ...
            if (!Constants.SCOPE_LOCAL.toString().equalsIgnoreCase(scope)) {
                if (logger.isInfoEnabled()) {
                    logger.info("Export bingo service " + interfaceClass.getName() + " to url " + url);
                }
                if (registryURLs != null && registryURLs.size() > 0
                    ...
                    for (URL registryURL : registryURLs) {
                        ...
                        if (logger.isInfoEnabled()) {
                            logger.info("Register bingo service " + interfaceClass.getName() + " url " + url + " to registry " + registryURL);
                        }
    ...
    private void exportLocal(URL url) {
        if (!Constants.LOCAL_PROTOCOL.equalsIgnoreCase(url.getProtocol())) {
            ...
            exporters.add(exporter);
            logger.info("Export bingo service " + interfaceClass.getName() + " to local registry");
        }
    }
