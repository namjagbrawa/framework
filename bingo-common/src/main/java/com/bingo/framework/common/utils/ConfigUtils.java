...
public class ConfigUtils {
    ...
    public static Properties getProperties() {
        if (PROPERTIES == null) {
            synchronized (ConfigUtils.class) {
                if (PROPERTIES == null) {
                    String path = System.getProperty(Constants.BINGO_PROPERTIES_KEY);
                    if (path == null || path.length() == 0) {
                        path = System.getenv(Constants.BINGO_PROPERTIES_KEY);
                        if (path == null || path.length() == 0) {
                            path = Constants.DEFAULT_BINGO_PROPERTIES;
                        }
                    }
	 ...
	 * 
	 * @param fileName properties file name. for example: <code>bingo.properties</code>, <code>METE-INF/conf/foo.properties</code>
    ...
    public static Properties loadProperties(String fileName, boolean allowMultiFile, boolean optional) {
        ...
        if(! allowMultiFile) {
            if (list.size() > 1) {
                String errMsg = String.format("only 1 %s file is expected, but %d bingo.properties files found on class path: %s",
                        fileName, list.size(), list.toString());
                logger.warn(errMsg);
                // throw new IllegalStateException(errMsg); // see http://code.alibabatech.com/jira/browse/BINGO-133
            }
